...
async function main() {
  ...
  `);
    ...
    for (const z of zones) {
        const res = await db_1.pool.query(`INSERT INTO zones(name) VALUES($1) RETURNING id`, [z]);
        zoneIds[z] = res.rows[0].id;
    }
    ...
    await db_1.pool.query(`INSERT INTO app_users(id, role, display_name) VALUES
      ($1,'supervisor','Supervisor'),
      ($2,'operator','Operator-A'),
      ($3,'operator','Operator-B')`, [supervisorId, operatorAId, operatorBId]);
    // Operator assignments:
    // Operator-A manages Zone-A and Zone-C, Operator-B manages Zone-B.
    await db_1.pool.query(`INSERT INTO operator_zones(operator_user_id, zone_id) VALUES
      ($1,$2), ($1,$3),
      ($4,$5)`, [operatorAId, zoneIds["Zone-A"], zoneIds["Zone-C"], operatorBId, zoneIds["Zone-B"]]);
    await db_1.pool.query(`INSERT INTO zone_operators(zone_id, operator_user_id) VALUES
      ($1,$2), ($3,$4), ($5,$2)`, [zoneIds["Zone-A"], operatorAId, zoneIds["Zone-B"], operatorBId, zoneIds["Zone-C"]]);
    ...
    const cfgThresholdRows = [];
    const cfgRateRows = [];
    const cfgAbsenceRows = [];
    ...
    for (const sensor of sensors) {
        ...
        const lastTsRes = await db_1.pool.query(`SELECT max(timestamp) as last_ts FROM readings WHERE sensor_id = $1`, [sensor.id]);
        const lastTs = lastTsRes.rows[0].last_ts;
        if (lastTs) {
            await db_1.pool.query(`
